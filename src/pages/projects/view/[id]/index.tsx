...
import { deleteSoilDataById } from 'apiSdk/soil-data';
...
function ProjectViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProjectInterface>(
    ...
    () =>
      getProjectById(id, {
        relations: ['excavator', 'soil_data'],
      }),
  );

  const soil_dataHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
          ...
          <>
            ...
            {hasAccess('soil_data', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
              <>
                ...
                <Link href={`/soil-data/create?project_id=${data?.id}`}>
                    ...
                    <Tbody>
                      {data?.soil_data?.map((record) => (
                        <Tr key={record.id}>
                          ...
                          <Td>
                            <Button>
                              <Link href={`/soil-data/edit/${record.id}`}>Edit</Link>
                            </Button>
                          </Td>
                          <Td>
                            <Button>
                              <Link href={`/soil-data/view/${record.id}`}>View</Link>
                            </Button>
                          </Td>
                          <Td>
                            <Button onClick={() => soil_dataHandleDelete(record.id)}>Delete</Button>
                          </Td>
                        </Tr>
                      ))}
                    </Tbody>
